...
export function generateFloorMap(floor: number): FloorMap {
  ...
  // Randomly place start and stairs, ensuring they're far enough apart
  const allIndices = Array.from({ length: total }, (_, i) => i);
  const startIdx = allIndices[rand(0, Math.floor(total / 2) - 1)]; // start in first half
  // stairs must be at least (rows + cols - 2) manhattan distance away
  const startRow = Math.floor(startIdx / cols);
  const startCol = startIdx % cols;
  const stairsCandidates = allIndices.filter(i => {
    const r = Math.floor(i / cols);
    const c = i % cols;
    return Math.abs(r - startRow) + Math.abs(c - startCol) >= Math.floor((rows + cols) / 2);
  });
  const stairsIdx = stairsCandidates.length > 0
    ? stairsCandidates[rand(0, stairsCandidates.length - 1)]
    : total - 1;
